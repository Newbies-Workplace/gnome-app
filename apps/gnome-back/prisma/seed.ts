...
async function main() {
  ...
  // const gnomesJson = await axios.get("https://visitwroclaw.eu/wp-json/custom/v1/attractions?ptype=dwarves")
  // const gnomes = gnomesJson.data
  // const listaId = []
  // for (const gnome of gnomes) {
  //   listaId.push(gnome.id)
  // }
  ...
  const gnomesJson = await axios.get(
    "https://visitwroclaw.eu/wp-json/custom/v1/attractions?ptype=dwarves",
  );
  const gnomesData = gnomesJson.data;
  for (const gnome of gnomesData) {
    const oneGnome = await axios.get(
      `https://visitwroclaw.eu/wp-json/custom/v1/attractions/${gnome.id}`,
    );
    const oneGnomeData = oneGnome.data;
    const districtId = await districtsService.findPointInPolygon([
      Number(oneGnomeData.lng),
      Number(oneGnomeData.lat),
    ]);
    const event = new Date(Date.now());
    await prisma.gnome.upsert({
      where: { name: oneGnomeData.title },
      update: {
        pictureUrl: oneGnomeData.image[0] || "",
        districtId: districtId,
      },
      create: {
        pictureUrl: oneGnomeData.image[0] || "",
        id: crypto.randomUUID(),
        name: oneGnomeData.title,
        latitude: Number(oneGnomeData.lat),
        longitude: Number(oneGnomeData.lng),
        location: oneGnomeData.address || "",
        creationDate: event.toISOString(),
        description: oneGnomeData.description || "",
        exists: true,
        funFact: "",
        districtId: districtId,
      },
